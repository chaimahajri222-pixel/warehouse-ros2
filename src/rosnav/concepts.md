...
**Avoidance was purely reactive** before §26.1 below: the moving obstacle was only ever seen through the live LaserScan → costmap `obstacle_layer`, so Nav2 replanned after the fact, same as for any static obstacle it hadn't seen yet. `obstacle_tracker.py`'s Kalman-filtered position/velocity tracks were logged/visualized but never fed into the costmap or planner.

### 26.1 Closing the loop: predictive avoidance (`dynamic_obstacle_predictor.py`, 2026-09-09)

`dynamic_obstacle_predictor.py` subscribes to `obstacle_tracker.py`'s `/obstacle_tracker/state` track feed, projects each track forward along its velocity (`predict_horizon_s`/`predict_steps`), and publishes a `nav_msgs/OccupancyGrid` mask marking each track's current footprint *and* its projected path — reusing the same "node publishes the mask directly, no `map_server`" trick `camera_terrain_speed_mask.py` (§37) already uses. Wired into both costmaps as a new `dynobs_keepout_filter` (`nav2_costmap_2d::KeepoutFilter`, `config/dynamic_obstacle_filter.yaml`) alongside the existing `gs_keepout_filter`/`gs_speed_filter` — inert (empty mask) until `dynamic_obstacles:=N>0` starts the whole chain (`obstacle_tracker.py` → `dynamic_obstacle_predictor.py` → `dynobs_costmap_filter_info_server` → `lifecycle_manager_dynobs`), started via the existing `dynamic_obstacles` arg, no new launch arg needed.

**Two real bugs found getting `obstacle_tracker.py` to actually work, both only visible once this feature made someone finally look at its output instead of just its code:**

1. **QoS mismatch (silent, zero data ever delivered).** `obstacle_tracker.py` subscribed to `/scan` with rclpy's default RELIABLE QoS; Gazebo's `/scan` bridge publishes BEST_EFFORT. An incompatible-QoS mismatch doesn't error — it just delivers nothing, forever, with no exception and no log line beyond `New publisher discovered on topic '/scan', offering incompatible QoS`. This node had therefore never been exercised against real scan data since it was written — every earlier "verified" claim about it was necessarily visualization/logic testing only. Fixed with `qos_profile_sensor_data` (BEST_EFFORT).

2. **No ego-motion compensation (only found because bug #1 was fixed).** The detector flagged a ray as an "approaching obstacle" whenever its raw range shrank between frames — with no correction for the robot's own motion. The instant real scan data started flowing during an actual `explore_lite` run, this aliased the robot's own translation/rotation onto every wall it drove toward: up to 9 simultaneous phantom tracks scattered meters apart, from 1 real obstacle in the world.

   Fix history (two dead ends before the working approach, all found live 2026-09-09):
   - `map -> base_link` TF at each scan's exact timestamp: correct frame in principle, but `map` (via SLAM/AMCL) updates far less often than the scan rate and consistently lagged behind each scan's own stamp (`Lookup would require extrapolation into the future` on nearly every callback) — starved the node of *any* detections, real or spurious.
   - `odom -> base_link` TF at each scan's exact timestamp: odom updates much faster than map, but still threw the same extrapolation error intermittently for gaps under 100ms — ordinary inter-process delivery jitter between the scan and TF pipelines is enough to break an exact-timestamp `lookup_transform`, since its `timeout` *waits* for data up to a future stamp rather than tolerating a small mismatch.
   - **What works:** subscribe to `/odom` directly, buffer `(stamp, x, y, cos_yaw, sin_yaw)` samples, and pick whichever buffered sample is *nearest* each scan's timestamp — a plain Python `min()` over already-received messages, no wait/extrapolation semantics to fail. The single `map <- odom` offset needed to publish in the map frame is still a live TF lookup, but only ever at "latest" (this node's original single-lookup style, which never had an extrapolation problem).

**Known limitation, not yet resolved (2026-09-09):** even with both bugs fixed plus added `min_cluster_points` (reject single/few-ray noise clusters) and `max_range` (6.0m default — ignore rays too far out to matter for local-costmap avoidance anyway) hardening, a ~90s headless `explore_lite` run still accumulated 6-9 simultaneous phantom tracks by the end, against 1 real obstacle. The pattern (false tracks concentrated at longer range, apparent speed roughly proportional to distance) points to nearest-timestamp odom matching accumulating yaw error under load — a small rotation-rate misestimate produces a large apparent tangential velocity far away but almost none nearby — likely worsened by this being a shared, loaded machine. Proper odom interpolation between the two bracketing samples (instead of nearest-match) is the most likely next step, untried so far.

**Given this, `dynobs_keepout_filter` should be treated as experimental, not production-safe path-blocking** — it only ever runs when `dynamic_obstacles:=N>0` is explicitly set (default `0`, same as before), so it can't affect a run that doesn't ask for it, but a long run *with* it enabled can currently paint spurious no-go zones. Fine for demoing "the wiring exists end-to-end" (confirmed live: both costmaps load `/dynobs/keepout_info` and `/dynobs/keepout_mask`, and a genuinely accelerating real obstacle is tracked correctly early in a run), not yet fine for actually trusting the avoidance it produces on a long run.
